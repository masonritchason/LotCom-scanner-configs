...
/**
 * PC Receiving Scanner Configuration.
 * 
...
function onResult(decodeResults, readerProperties, output) {
    ...
    // validate that the first field is a PO Number
    if (!validatePONumber(processedResults[0])) {
        // not a PO number; throw an error 
        output = dataValidationError(output, "Invalid Supplier Master Label or PO Number.");
        return;
    }
    ...
    if (!validatePartNumber(processedResults[1])) {
        ...
        output = dataValidationError(output, "Invalid Supplier Master Label or Part Number.");
        return;
    }
    ...
    if (!validateQuantity(processedResults[3])) {
        ...
        output = dataValidationError(output, "Invalid Supplier Master Label or Total Quantity.");
        return;
    }

    // validate that the fifth field is a JBK #
    if (!validateJBKNumber(processedResults[4])) {
        // not a JBK number; throw an error 
        output = dataValidationError(output, "Invalid Supplier Master Label or JBK Number.");
        return;
    }
    ...
    output.OLED = "Supplier Master Label captured successfully.";
}
