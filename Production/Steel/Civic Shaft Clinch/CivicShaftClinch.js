...
function onResult(decodeResults, readerProperties, output) {
    ...
    // if the label wasn't marked as INHOUSE and this block is reached, the scanner does accept supplier parts
    if (labelOriginType != "INHOUSE") {
        // validate the supplier component label formats
        var validation = validateSupplierLabel(processedResults, output);
        // if the label wasn't valid, throw the error generated by the method
        if (!validation[0]) {
            output = validation[1];
            return;
        }
        // valid format for supplier label; confirm that the part number is an acceptable part number
        if (!isStringInArray(acceptedSupplierPartNumbers, processedResults[0])) {
        ...
        // the label was a supplier label and was valid; update origin flag
    ...
    if (labelOriginType != "SUPPLIER") {
        ...
        if (!validatePartNumber(processedResults[1])) {
            output = dataValidationError(output, "Invalid Uppershaft MC Label or Invalid Part Number: " + processedResults[1]);
            return;
        }
...
/**
 * Validates a processed scan result as a valid Supplier Label.
 * @param {string[]} processedResults A Processed result array.
 * @param {any} output the `output` module generated by the Scanner.
 * @returns {[bool, any]} the boolean result of the check and a modified `output` module (for errors).
 */
function validateSupplierLabel(processedResults, output) {
    // validate that the first field is a Part Number
    if (!validatePartNumber(processedResults[0])) {
        output = dataValidationError(output, "Invalid Supplier Component Part Number. Please scan a valid Supplier Component Label.")
        return false, output;
    }
    // validate that the third field is a Quantity
    if (!validateQuantity(processedResults[2])) {
        output = dataValidationError(output, "Invalid Supplier Component Quantity. Please scan a valid Supplier Component Label.")
        return false, output;
    }
    // validate that the fourth field is a Lot Number
    if (!validateLotNumber(processedResults[3])) {
        output = dataValidationError(output, "Invalid Supplier Component Lot Number. Please scan a valid Supplier Component Label.")
        return false, output;
    }
