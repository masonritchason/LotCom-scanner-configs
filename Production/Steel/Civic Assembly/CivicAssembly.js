...
function onResult(decodeResults, readerProperties, output) {
    ...
    if (!isStringInArray(allowedPartNumbers, processedResults[1])) {
        output = dataValidationError(output, "Invalid Label. Please scan a Civic Process Label.");
        return;
    }
    ...
    if (isStringInArray(allowedProcesses, processedResults[0])) {
        ...
        // supplier Label
        labelOrigin = "SUPPLIER";
    }

    // if YNA In-House, perform consolidations
    if (labelOrigin == "YNA") {
    ...
    // if YNA In-House, perform validations
    if (labelOrigin == "SUPPLIER") {
        ...
        if (!validateQuantity(processedResults[2])) {
            output = dataValidationError(output, "Invalid WIP Label or Invalid Quantity.");
            return;
        }
        ...
        if (processedResults[0] == "PivotHousingMC") {
            ...
            if (!validateJBKNumber(processedResults[3])) {
    ...
    // if Supplier, do validations
    if (labelOrigin == "SUPPLIER") {
        /**
         * Validate Supplier Labels here
         */
        output = dataValidationError(output, "Invalid WIP Label.");
        return;
    }
